...
def get_scurve(dev, resolution=50):
  ...
  return S_Curve(dev, current, num_to_avg=500)
...
def dist_rng(dev, k, init, lmda, dump_mod_val, mag_view_flag, file_ID, scurve, pdf_type):
  if pdf_type == "exp":
    x2 = 2**k
    x0 = 1
    x1 = (x2+x0)/2
    ...
    x0 = GAMMA_LOWER
    x1 = (x2+x0)/2
    xxis = np.linspace(GAMMA_LOWER, GAMMA_UPPER, RESOLUTION)
    cdf_arr = stats.gamma.cdf(xxis, a=50, scale=1/311.44)
    cdf = lambda x,lmda: cdf_arr[round(remap(x, GAMMA_LOWER, GAMMA_UPPER, 0, len(cdf_arr)-1))]
...
def get_energy(dev, samples, scurve, pdf_type):
  k = 8
  lmda = 0.01
  init_t = 9*np.pi/10
  number_history = []
