...
class SOT_Env(Env):
  def __init__(self):
    ...
    while(True):
      ...
      chi2, bitstream, energy_avg, countData, bitData, xxis, exp_pdf = SOT_Model(self.alpha, self.Ki, self.Ms, self.Rp, self.TMR, self.d, self.tf, self.eta, self.J_she, self.t_pulse, self.t_relax, samples=self.dev_samples)
      if chi2 != None:
        break
