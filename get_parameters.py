...
# d_vals       = [3e-09]                                  # free layer diameter
# tf_vals      = [1.1e-09]                                # free layer thickness
...
alpha_vals   = [0.01, 0.05, 0.1]        # damping constant
Ki_vals      = [0.2e-3, 0.6e-3, 1e-3]   # anistrophy energy    
Ms_vals      = [0.3e6, 1.2e6, 2e6]      # saturation magnetization
Rp_vals      = [500, 5000, 50000]       # parallel resistance
TMR_vals     = [0.3, 2, 6]              # tunneling magnetoresistance ratio
d_vals       = [3e-09]                  # free layer diameter
tf_vals      = [1.1e-09]                # free layer thickness
eta_vals     = [0.1, 0.4, 0.8]          # spin hall angle
J_she_vals   = [0.01e12, 0.25e12, 1e12] # current density
t_pulse_vals = [0.5e-9, 10e-9, 75e-9]   # pulse duration      
              ...
              for eta in eta_vals:
                for J_she in J_she_vals:
                  for t_pulse in t_pulse_vals:
                    t_relax = t_pulse
                    p = [alpha, Ki, Ms, Rp, TMR, d, tf, eta, J_she, t_pulse, t_relax]
                    parameters.append(p)
